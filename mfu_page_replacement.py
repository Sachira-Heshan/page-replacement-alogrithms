...
# function for implement most frequently used algorithm
def mfu_page_replacement():
        ...
        if reference_sequence[i] in temp_frame_block:
            ...
            for j in range(0, len(temp_frame_block)):
                if temp_frame_block[j] != 0:
                    ...
                    # if the frame block have unassigned frames, freqeuency of that set to 0
                    temp_frequencies.append(0) 

            temp_max_freq = max(temp_frequencies)
            flag = 1
            ...
            max_freq_pages = []
            # check if there other max frequecies in the that frame block
            for element in temp_frame_block:
                if element != 0:
                    if page_frequencies[element] == temp_max_freq:
                        max_freq_pages.append(element)
                else:
                    flag = 0
                    temp_frame_block[temp_frame_block.index(element)] = reference_sequence[i]
                    break

            # if there are more than one page with same max frequency then go to FIFO
            if len(max_freq_pages) > 1 and flag:
                for page in in_out_seq:
                    if page in max_freq_pages:
            ...
            elif len(max_freq_pages) == 1 and flag:
                temp_frame_block[temp_frame_block.index(max_freq_pages[0])] = reference_sequence[i]
    ...
    print(f"Min Freq Pages: {max_freq_pages}")
